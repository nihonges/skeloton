...
def main(opt, train_dataset, test_dataset):
    """Main methode to call baseline model"""
    start_epoch = 0
    err_best = 1000
    glob_step = 0
    lr_now = opt.lr
    ...
    for data in train_loader:
        imgs, d2, d3 = data['image'], data['key_points_2d'], data['key_points_3d']
        tb.add_images("step", imgs, step)
        step += 1
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        loss_test, err_test = test(
            test_loader, model, criterion, procrustes=opt.procrustes)

        tb.add_scalar('Train_Loss', loss_train, epoch)
        tb.add_scalar('Test_Loss', loss_test, epoch)
        tb.add_scalar('Error', err_test, epoch)
...
def train(train_loader, model, criterion, optimizer,
    ...
    for i, mydict in enumerate(train_loader):
        inps = mydict['key_points_2d'].flatten(start_dim=1).float()
        tars = mydict['key_points_3d'].flatten(start_dim=1).float()
        glob_step += 1
...
def test(test_loader, model, criterion, procrustes=False):
    ...
    for i, mydict in enumerate(test_loader):
        ...
        ########################################################################
        ########################################################################
        outputs_use = outputs.data.cpu().numpy()
